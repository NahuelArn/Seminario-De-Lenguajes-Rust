...
impl Veterinaria {
    ...
    fn impactar_en_archivo(&self) {
        let msg = "No se pudo guardar en el archivo";
        let json = serde_json::to_string(&self.registros).expect(msg);
        let mut file = fs::File::create("src/tp5/veterinaria.json").expect(msg);
        fs::write("src/tp5/veterinaria.json", json).expect(msg);
    }
    fn cargar_desde_archivo() -> Result<Vec<Registro> > {
        let msg = "No se pudo leer el archivo";
        let file = fs::read_to_string("src/tp5/veterinaria.json").expect(msg);
        let registros: Vec<Registro> = serde_json::from_str(&file).expect(msg);
        Ok(registros)
    }
}
//=====================================Fecha===================================================
...
fn test_modificar_fecha_proxima_visita() {
    let mut veterinaria = Veterinaria::new(String::from("Vet8"), String::from("Direccion 8"), 8);
    let mascota = Mascota::new(
        String::from("Mascota1"),
        5,
        Tipo_Animal::Perro,
    ...
    let fecha = Fecha::new(1, 1, 2022);
    let modificado = veterinaria.modificar_fecha_proxima_visita(mascota, fecha);
    assert_eq!(modificado, true);
}
#[test]
fn test_eliminar_atencion_registro() {
    let mut veterinaria = Veterinaria::new(String::from("Vet9"), String::from("Direccion 9"), 9);
    let mascota = Mascota::new(
        String::from("Mascota1"),
        5,
        Tipo_Animal::Perro,
    ...
    let eliminado = veterinaria.eliminar_atencion_registro(mascota);
    assert_eq!(eliminado, true);
}
#[test]
fn test_es_bisiesto() {
...
fn test_cantidad_de_dias() {
    let fecha = Fecha::new(1, 1, 2020);
    assert_eq!(fecha.cantidad_de_dias(2), 29);
    let fecha = Fecha::new(1, 1, 2021);
    assert_eq!(fecha.cantidad_de_dias(2), 28);
    let fecha = Fecha::new(1, 1, 2020);
    assert_eq!(fecha.cantidad_de_dias(1), 31);
    let fecha = Fecha::new(1, 1, 2020);
    assert_eq!(fecha.cantidad_de_dias(4), 30);
}
#[test]
fn test_es_fecha_valida() {
    let fecha = Fecha::new(1, 1, 2020);
    assert_eq!(fecha._es_fecha_valida(), true);
    let fecha = Fecha::new(1, 13, 2020);
    assert_eq!(fecha._es_fecha_valida(), false);
    let fecha = Fecha::new(32, 1, 2020);
    assert_eq!(fecha._es_fecha_valida(), false);
    let fecha = Fecha::new(29, 2, 2021);
    assert_eq!(fecha._es_fecha_valida(), false);
}
#[test]
fn test_sumar_dias() {
    ...
    let mut fecha = Fecha::new(28, 2, 2021);
    fecha._sumar_dias(1);
    assert_eq!(fecha, Fecha::new(1, 3, 2021));
}
#[test]
fn test_restar_dias() {
    let mut fecha = Fecha::new(2, 1, 2020);
    fecha.restar_dias(1);
    assert_eq!(fecha, Fecha::new(1, 1, 2020));
    let mut fecha = Fecha::new(1, 1, 2021);
    fecha.restar_dias(1);
    assert_eq!(fecha, Fecha::new(31, 12, 2020));
    let mut fecha = Fecha::new(29, 2, 2020);
    fecha.restar_dias(1);
    assert_eq!(fecha, Fecha::new(28, 2, 2020));
    let mut fecha = Fecha::new(1, 3, 2021);
    fecha.restar_dias(1);
    assert_eq!(fecha, Fecha::new(28, 2, 2021));
}
#[test]
fn test_es_mayor() {
    let fecha1 = Fecha::new(1, 1, 2020);
    let fecha2 = Fecha::new(2, 1, 2020);
    assert_eq!(fecha2._es_mayor(&fecha1), true);
    let fecha1 = Fecha::new(1, 1, 2020);
    let fecha2 = Fecha::new(1, 2, 2020);
    assert_eq!(fecha2._es_mayor(&fecha1), true);
    let fecha1 = Fecha::new(1, 1, 2020);
    let fecha2 = Fecha::new(1, 1, 2021);
    assert_eq!(fecha2._es_mayor(&fecha1), true);
}
#[test]
fn test_to_string() {
    let fecha = Fecha::new(1, 1, 2020);
    assert_eq!(fecha.to_string(), "dia: 1 mes: 1 anho: 2020");
}