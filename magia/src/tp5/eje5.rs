...
fn test_cantidad_de_dias() {
    ...
    assert_eq!(fecha_bisiesta.cantidad_de_dias(2), 29);

    // Febrero en año no bisiesto
    assert_eq!(fecha_no_bisiesta.cantidad_de_dias(2), 28);

    // Mes inválido
    assert_eq!(fecha_bisiesta.cantidad_de_dias(13), 0);
}

#[test]
fn test_cantidad_de_dispositivos() {
    assert_eq!(TipoSuscripcion::Basic.cantidad_de_dispositivos(), 1);
    assert_eq!(TipoSuscripcion::Clasic.cantidad_de_dispositivos(), 2);
    assert_eq!(TipoSuscripcion::Super.cantidad_de_dispositivos(), 4);
}